# Script 07: Overlay California FRAP Fire Perimeters on Scanned Maps
# -------------------------------------------------------------
...
# Saves three separate JPEG representation maps in the output/ directory.
...
log_msg("Starting Script 07: Overlaying FRAP wildfires on California maps...")
...
# Let's write a function to generate a JPEG representation for each map
process_map_frap <- function(map_name, map_path) {
  ...
  jpeg(output_jpg, width = original_w, height = original_h)
  
  # Set margins to 0 to maximize map coverage and keep clean edges
  par(mar = c(0, 0, 0, 0))
  
  # Plot the RGB scanned map base
  terra::plotRGB(r, r = 1, g = 2, b = 3, mar = c(0, 0, 0, 0))
