...
w1 <- ext1[2] - ext1[1]
h1 <- ext1[4] - ext1[3]
r1_cropped <- crop(r1, ext(ext1[1] + 200 + 0.1*w1, ext1[2] - 200 - 0.1*w1, ext1[3] + 200 + 0.1*h1, ext1[4] - 200 - 0.1*h1))

ext2c <- ext(r2_cropped)
w2 <- ext2c[2] - ext2c[1]
h2 <- ext2c[4] - ext2c[3]
r2_cropped_final <- crop(r2_cropped, ext(ext2c[1] + 200 + 0.1*w2, ext2c[2] - 200 - 0.1*w2, ext2c[3] + 200 + 0.1*h2, ext2c[4] - 200 - 0.1*h2))

# Recalculate native CRS from cropped r1
native_crs <- crs(r1_cropped)
...
# We downsample cropped native rasters directly without any reprojection/resampling
r1_sub_std <- aggregate(r1_cropped, fact = 5)
r2_sub_std <- aggregate(r2_cropped_final, fact = 5)
...
# Use the combined extent directly without padding for a tighter crop
...
# We use the cropped native rasters directly without reprojection or resampling!
r1_hr <- r1_cropped
r2_hr <- r2_cropped
