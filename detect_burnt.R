...
burned_path <- "/home/coder/forest_types_r/data/burned.jpg"
if (!file.exists(burned_path)) {
  # Try burnt.jpg just in case
  burned_path <- "/home/coder/forest_types_r/data/burnt.jpg"
}
...
detect_burnt <- function(map_path, output_path) {
  ...
  # Downsample for faster processing (factor of 4)
  fact <- 4
  map_small <- aggregate(map, fact = fact)
...
target_map <- "/home/coder/forest_types_r/data/forest_maps/Plate_CXXVI.jpg"
output_file <- file.path(output_dir, "burnt_Plate_CXXVI.tif")
...
map <- rast(target_map)
png_output <- file.path(output_dir, "burnt_plot_CXXVI.png")

png(png_output, width=1200, height=1000)
...
dev.off()
print(paste("Plot saved to", png_output))
