...
for (rf in raster_files) {
  ...
  # Downsample by a factor of 10 for speed and memory efficiency
  r_sub <- aggregate(r, fact = 10)
...
# 4. Define plot extent based on Oregon counties
ext_3857 <- ext(or_counties_3857)
...
output_path <- file.path(output_dir, "oregon_composite_map.png")

# 6. Plotting to high-resolution large image
print(paste("Plotting composite to:", output_path))
png(output_path, width = 2400, height = 1800, res = 200)

# Start with empty plot of Oregon counties to establish coordinate system and extent
...
for (name in names(projected_rasters)) {
  ...
  # Use tryCatch to ensure we don't crash if one raster has plotting issues
  tryCatch({
